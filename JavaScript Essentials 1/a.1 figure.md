...
%% Core Idea
A[JavaScript Runs on Both Client & Server]
...
%% Client side
B --> B1[Handles UI & User Interaction]
B --> B2[Runs on User Device]
B --> B3[Sends Requests to Server]

%% Server side
C --> C1[Handles Data & Business Logic]
...
C --> J[Backend Code is Hidden & Secure]
J --> J1[Authentication]
J --> J2[Security Rules]
...
K --> K2[Direct API-Focused Development]
...
%% Common mistakes
M[Common Mistakes]
M --> M1[Mixing Frontend & Backend Logic]
M --> M2[Focusing Too Much on UI]
M --> M3[Ignoring Server-Side Thinking]
M --> M4[Fix: Focus on Requests, Data, Responses]
```
